import { IsDateString, IsISO8601, IsEnum, IsNotEmpty, IsOptional, IsString, IsUUID } from 'class-validator';
...
export class CreateTaskDto {
  ...
  @IsISO8601()
  @Transform(({ value }) => {
    // If a Date object is sent, convert to ISO string
    if (value instanceof Date) return value.toISOString();
    // If a number (timestamp) is sent, convert to ISO string
